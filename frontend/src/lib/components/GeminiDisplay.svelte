...
<div class="flex flex-col gap-4">
	<div class="flex flex-row gap-4 rounded-xl bg-warn-200 p-4">
	...
	<div class="mx-auto w-full overflow-hidden rounded-xl border border-gray-200 bg-white">
		<div class="space-y-6 p-6">
			...
			<div class="grid grid-cols-2 gap-4 text-sm text-gray-800">
				...
				<div>
					<span class="font-semibold">Vollständig:</span>
					{info.ist_komplett_vollständig ? 'Ja' : 'Nein'}
				</div>
				...
				<div>
					...
					<div class="mt-1 flex flex-wrap gap-2">
						{#each info.komposita_teile as teil}
							<span class="rounded bg-gray-100 px-2 py-1 text-sm text-gray-700">{teil}</span>
						{/each}
					</div>
				</div>
			{/if}
			...
			{#if info.substantiv_morphologie}
				<MorphologyBlock
					...
					entries={[
						...
						[
							'Stammänderung',
							info.substantiv_morphologie.unregelmäßige_stammänderung ? 'Ja' : 'Nein'
						],
			...
			{#if info.adjektiv_morphologie}
				<MorphologyBlock
					...
					entries={[
						...
						[
							'Stammänderung',
							info.adjektiv_morphologie.unregelmäßige_stammänderung ? 'Ja' : 'Nein'
						],
